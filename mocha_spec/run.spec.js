...
describe('Integration Test', () => {
    ...
    describe.only('AMQP ack() method integration tests', () => {
        describe('when connection is closed after receiving message and then reconnected', () => {
            it('should ack message with new consumerTag after reconnection', async function test() {
                ...
                // Setup AMQP instance
                const amqp = new Amqp({
                    AMQP_URI: env.ELASTICIO_AMQP_URI,
                    MESSAGE_CRYPTO_PASSWORD: env.ELASTICIO_MESSAGE_CRYPTO_PASSWORD,
                    MESSAGE_CRYPTO_IV: env.ELASTICIO_MESSAGE_CRYPTO_IV,
                    AMQP_PUBLISH_RETRY_ATTEMPTS: 1,
                    AMQP_PUBLISH_RETRY_DELAY: 100,
                    AMQP_PUBLISH_MAX_RETRY_DELAY: 1000
                });

                const testQueue = `test_ack_queue_${Date.now()}`;
                ...
                try {
                    ...
                    // Publish a test message to receive
                    const testMessage = {
                        body: { test: 'data' },
                        headers: { messageId }
                    };

                    const encryptedContent = amqp._encryptor.encryptMessageContent(testMessage, 'utf8');
                    await amqp.publishChannel.assertQueue(testQueue);
                    await amqp.publishChannel.sendToQueue(testQueue, encryptedContent, {
                        headers: {
                            messageId,
                            protocolVersion: 1
                        }
                    ...
                    // Start consuming and get the message
                    let originalConsumerTag;
                    const messagePromise = new Promise((resolve) => {
                        amqp.listenQueue(testQueue, (decryptedContent, message) => {
                            // Store original consumerTag
                            originalConsumerTag = message.fields.consumerTag;

                            // Add message to messagesDB (this is what sailor does internally)
                            messagesDB.addMessage(messageId, message);

                            resolve({ decryptedContent, message });
                        });
                    });

                    const { message: originalMessage } = await messagePromise;

                    // Get the message via getMostRecentMessage (should return immediately since consumerTag matches)
                    const receivedMessage1 = await amqp.getMostRecentMessage(messageId);
                    expect(receivedMessage1.fields.consumerTag).to.equal(originalConsumerTag);

                    // Close the consumer connection to simulate disconnection
                    await amqp.stopConsume();
                    await amqp.consumerChannel.close();
                    await amqp._readConnection.close();

                    // Reconnect (this will create a new consumerTag)
                    await amqp._ensureConsumerChannel();

                    // Start consuming again with new connection
                    const newMessagePromise = new Promise((resolve) => {
                        amqp.listenQueue(testQueue, (decryptedContent, message) => {
                            // Update message in messagesDB with new consumerTag
                            messagesDB.addMessage(messageId, message);
                            resolve({ decryptedContent, message });
                        });
                    });

                    // Re-send the same message to simulate it being delivered with new consumerTag
                    await amqp.publishChannel.sendToQueue(testQueue, encryptedContent, {
                        headers: {
                            messageId,
                            protocolVersion: 1
                    ...
                    const { message: newMessage } = await newMessagePromise;
                    const newConsumerTag = newMessage.fields.consumerTag;

                    // Verify we have a different consumerTag
                    expect(newConsumerTag).to.not.equal(originalConsumerTag);

                    // Now getMostRecentMessage should wait for and return the message with new consumerTag
                    const receivedMessage2 = await amqp.getMostRecentMessage(messageId);
                    expect(receivedMessage2.fields.consumerTag).to.equal(newConsumerTag);

                    // Finally, ack the message - this should work with the new consumerTag
                    await amqp.ack(messageId);

                    // Verify message was deleted from messagesDB after ack
                    const deletedMessage = messagesDB.getMessageById(messageId);
                    expect(deletedMessage).to.be.undefined;
                    ...
                    // Cleanup
                    try {
                        if (amqp.publishChannel) {
                            await amqp.publishChannel.deleteQueue(testQueue);
                        }
