...
describe('AMQP', () => {
    ...
    it('Should send message to outgoing channel after 3 attempts', async () => {
        const amqp = new Amqp(settings);
        let iteration = 0;
        amqp.publishChannel = {
            on: sandbox.stub(),
        ...
        expect(amqp.publishChannel.publish).to.have.been.callCount(4);
        expect(amqp.publishChannel.publish).to.have.been.calledWith(
            settings.PUBLISH_MESSAGES_TO,
            settings.DATA_ROUTING_KEY,
            sinon.match(arg => {
    ...
    it('Should throw error after 3 attempts to publish message', async () => {
        const amqp = new Amqp(settings);
        amqp.publishChannel = {
            on: sandbox.stub(),
            publish: sandbox.stub()
        ...
        expect(caughtError).to.be.instanceof(Error);
        expect(amqp.publishChannel.publish).to.have.been.callCount(10);
        expect(amqp.publishChannel.publish).to.have.been.calledWith(
            settings.PUBLISH_MESSAGES_TO,
            settings.DATA_ROUTING_KEY,
            sinon.match(arg => {
