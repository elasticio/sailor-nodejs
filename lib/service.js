...
function processService(serviceMethod, env) {
  ...
  function verifyCredentials(cfg, params) {
    ...
    async function verifyCredentials(cfg, params) {
        const verify = await compReader.loadVerifyCredentials();

        const caller = new EventEmitter();
        caller.logger = new ComponentLogger();

        let response;
        let promiseResponse;

        try {
            response = await verify.call(caller, cfg, (e, verification) => {
                if (e) {
                    console.log(e);
                    throw e;
                } else if (verification) {
                    promiseResponse = verification;
                    return verification;
                }
            });
        } catch (e) {
            console.log("SAILOR HAS NOTICED THE ERROR BING BING BING")
            throw e;
        }
        
        if (!_.has(response, 'verified')) {
            response = promiseResponse || { verified: true };
        }

        return response || promiseResponse;
    }
  ...
  function callModuleMethod(triggerOrAction, method, cfg) {
    ...
    function onUpdateKeys(keys) {
      ...
      addPromise(apiClient.accounts.update(cfg._account, { keys }));
    }
