...
class Amqp {
    ...
    async maesterGet(maesterId) {
        ...
        return (await request.get(`${this.settings.MAESTER_BASEPATH}/objects/${maesterId}`, options)).body;
    }

    async maesterPut(maesterId, data) {
        const options = {
            auth: {
                bearer: this.settings.MAESTER_JWT
            },
        ...
        s.push(data);
        s.push(null);
        return await s.pipe(request.put(`${this.settings.MAESTER_BASEPATH}/objects/${maesterId}`, options));
    }
