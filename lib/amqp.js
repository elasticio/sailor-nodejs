...
class Amqp {
    ...
    decryptMessage(callback, message) {
        ...
        try {
            ...
            // TODO: maybe find a way to pass messageId and also wait for new
            // message in case original connection was closed
    ...
    async getMostRecentMessage(messageId) {
        ...
        const isOldMessage = message.fields.consumerTag !== this.consume.consumerTag;
        if (!isOldMessage) {
            return message;
        }
        log.debug({ messageId }, 'Waiting for message from new channel');
        while (message.fields.consumerTag !== this.consume.consumerTag) {
            await new Promise(resolve => setImmediate(resolve));
            message = messagesDB.getMessageById(messageId);
        }
    ...
    async ack(messageId) {
        ...
        log.debug(message.fields, 'Message fields');
