...
class Amqp {
    ...
    async decryptMessage(message) {
        ...
        return new Promise(async (resolve, reject) => {
            const s = request
                .get(`${this.settings.OBJECT_STORAGE_URI}/objects/${objectId}`, options)
                .on('response', (res) => {
                    if (res.statusCode >= 400) {
                        reject(new Error(`HTTP error during object get: ${res.statusCode} (${res.statusMessage})`));
                    }
                })
                .on('error', reject);
            resolve(await encryptor.decryptMessageContentStream(s));
        });
    }
    ...
    async encryptMessage(data, properties) {
        ...
        await new Promise((resolve, reject) => {
            const res = request
                .put(`${this.settings.OBJECT_STORAGE_URI}/objects/${objectId}`, options)
                .on('response', (res) => {
                    if (res.statusCode >= 400) {
                        reject(new Error(`HTTP error during object put: ${res.statusCode} (${res.statusMessage})`));
                    }
                })
                .on('error', reject);
