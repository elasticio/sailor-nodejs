...
AMQPConnection.prototype.sendError = function sendError(err, headers, originalMessageContent) {
    ...
    var routingKey = getRoutingKeyFromHeaders(headers) || settings.ERROR_ROUTING_KEY;

    if (routingKey !== settings.ERROR_ROUTING_KEY) {
        options.headers[HEADER_ERROR_RESPONSE] = true;
    }

    this.sendToExchange(settings.PUBLISH_MESSAGES_TO, routingKey, errorPayload, options);
};
