...
class Amqp {
        ...
        while (true) {
            ...
            try {
                reads = await Promise.all(
                    (new Array(this.settings.RABBITMQ_PREFETCH_SAILOR)).fill().map(() =>
                        channel.get(queue))
                );
            } catch (e) {
                if (this.closed) {
    ...
    async _createConnection() {
        ...
        let connection;
        for (let i = 0; i < parseInt(this.settings.AMQP_RECONNECT_ATTEMPTS); i++) {
            try {
                ...
                if (this.reconnectCount >= parseInt(this.settings.AMQP_RECONNECT_ATTEMPTS)) {
                    log.debug(
                        {
    ...
    async listenQueue(queue, messageHandler) {
        ...
        for await (const batch of this._qBatchReader(queue)) {
            await Promise.all(batch.map(async amqpMessage => {
                let message;
                try {
