...
class Amqp {
    ...
    async decryptMessage(message) {
        ...
        if (objectId) {
            const options = {
                auth: { bearer: this.settings.OBJECT_STORAGE_TOKEN },
                forever: true,
                maxAttempts: 3,
                retryDelay: 100
            };
            const res = (await request.get(`${this.settings.OBJECT_STORAGE_BASEPATH}/objects/${objectId}`, options));
            if (res.statusCode >= 400) {
                throw new Error(`HTTP error during object get: ${res.statusCode} (${res.statusMessage})`);
            }
            content = res.body;
        }
    ...
    async encryptMessage(data, properties) {
        ...
        if (!this.settings.OBJECT_STORAGE_OUT) {
        ...
        const options = {
            ...
            retryStrategy: (err, response, body, options) => {
                if (!err && response.statusCode === 409) {
                    objectId = uuid.v4();
                    options.url = `${this.settings.OBJECT_STORAGE_BASEPATH}/objects/${objectId}`;
        ...
        const res = await request.put(`${this.settings.OBJECT_STORAGE_BASEPATH}/objects/${objectId}`, options);
        if (res.statusCode >= 400) {
            throw new Error(`HTTP error during object put: ${res.statusCode} (${res.statusMessage})`);
        }
        properties.headers.objectId = objectId;
