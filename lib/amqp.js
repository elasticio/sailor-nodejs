...
const HEADER_EXCHANGE = 'x-eio-exchange';
...
class Amqp {
    ...
    async prepareMessageAndSendToExchange(data, properties, routingKey, throttle) {
        const settings = this.settings;
        const exchange = getExchangeFromHeaders(data.headers) || settings.PUBLISH_MESSAGES_TO;
        ...
        return this.sendToExchange(exchange, routingKey, encryptedData, properties, throttle);
    }
...
// FIXME test
function getExchangeFromHeaders(headers) {
    if (!headers) {
        return null;
    }
    ...
    return lowerCaseHeaders[HEADER_EXCHANGE];
...
function filterMessageHeaders(headers = {}) {
    return _.transform(headers, (result, value, key) => {
        if ([HEADER_ROUTING_KEY, HEADER_EXCHANGE].includes(key.toLowerCase())) {
            return;
        }
        result[key] = value;
    }, {});
