...
class Amqp {
    ...
    async maesterGet(messageId) {
        const options = {
            auth: {
                bearer: this.settings.MAESTER_JWT
            },
            // TODO: check agent works as expected
            forever: true,
            // TODO: retry options
        };
        return await request.get(`${this.settings.MAESTER_BASEPATH}/objects/${messageId}`, options).body;
    }

    async maesterPut(messageId, data) {
        const options = {
            auth: {
                bearer: this.settings.MAESTER_JWT
            },
        ...
        s.push(data);
        s.push(null);
        return await s.pipe(request.put(`${this.settings.MAESTER_BASEPATH}/objects/${messageId}`, options));
    }
    ...
    listenQueue(queueName, callback) {
        ...
        return this.subscribeChannel.consume(queueName, async function decryptMessage(message) {
            ...
            // TODO: handle errors
            // TODO: check maesterId flag is not copied to outgoing message
            const content = message.properties.headers.maesterId ? await self.maesterGet(message.properties.headers.maesterId) : message.content;
            let decryptedContent;
            try {
                decryptedContent = encryptor.decryptMessageContent(content);
            } catch (err) {
                console.error(
    ...
    async prepareMessageAndSendToExchange(data, properties, routingKey, throttle) {
        ...
        if (this.settings.MAESTER_IS_STORE) {
            const maesterId = uuid.v4();
            // TODO: handle errors
            await this.maesterPut(maesterId, encryptedData);
            properties.headers.maesterId = maesterId;
        }

        return this.sendToExchange(settings.PUBLISH_MESSAGES_TO, routingKey, this.settings.MAESTER_IS_STORE ? '' : encryptedData, properties, throttle);
    }
