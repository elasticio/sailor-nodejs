...
class Amqp {
    ...
    async stopConsume() {
        // TODO: do we need to check whether it exists?
        const consumerTag = this.consume.consumerTag;
        this.consume = undefined;
        await this.consumerChannel.cancel(consumerTag);
    }

    // TODO: can it be called twice from outside? then we will have 2 listeners and lose consumer tag of one of them
    async listenQueue(queue, messageHandler) {
        await this._ensureConsumerChannel();
        this.consume = { queue, messageHandler };

        this.consume.consumerTag = await this.consumerChannel.consume(queue, async (amqpMessage) => {
            let message;
            try {
