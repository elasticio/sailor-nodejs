...
var encryptor = require('./cipher.js');
...
AMQPConnection.prototype.listenQueue = function listenQueue(queueName, callback) {
    ...
    this.subscribeChannel.prefetch(settings.get('RABBITMQ_PREFETCH_SAILOR'));
...
AMQPConnection.prototype.sendData = function sendData(data, headers) {
    ...
    this.sendToExchange(settings.get('PUBLISH_MESSAGES_TO'), settings.get('DATA_ROUTING_KEY'), encryptedData, options);
};
...
AMQPConnection.prototype.sendError = function sendError(err, headers, originalMessageContent) {
    ...
    this.sendToExchange(settings.get('PUBLISH_MESSAGES_TO'), settings.get('ERROR_ROUTING_KEY'), errorPayload, options);
};
...
AMQPConnection.prototype.sendRebound = function sendRebound(reboundError, originalMessage, headers) {
    ...
    if (reboundIteration > settings.get('REBOUND_LIMIT')) {
    ...
    function getExpiration(iteration) {
        return Math.pow(2, iteration - 1) * settings.get('REBOUND_INITIAL_EXPIRATION');
    }
};

AMQPConnection.prototype.sendSnapshot = function sendSnapshot(data, headers) {
    var options = getOptions(headers);
    var exchange = settings.get('PUBLISH_MESSAGES_TO');
    var routingKey = settings.get('SNAPSHOT_ROUTING_KEY');
    try {
