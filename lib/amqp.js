...
class Amqp {
    ...
    async decryptMessage(message) {
        ...
        if (objectId) {
            ...
            const res = (await request.get(`${this.settings.OBJECT_STORAGE_URI}/objects/${objectId}`, options));
    ...
    async encryptMessage(data, properties) {
        ...
        const options = {
            ...
            retryStrategy: (err, response, body, options) => {
                if (!err && response.statusCode === 409) {
                    objectId = uuid.v4();
                    options.url = `${this.settings.OBJECT_STORAGE_URI}/objects/${objectId}`;
        ...
        const res = await request.put(`${this.settings.OBJECT_STORAGE_URI}/objects/${objectId}`, options);
        if (res.statusCode >= 400) {
