...
class Amqp {
    ...
    async publishMessage(exchangeName, routingKey, payloadBuffer, options, iteration) {
        ...
        try {
            ...
            const currentDelay = settings.AMQP_PUBLISH_RETRY_DELAY;
            const maxDelay = settings.AMQP_PUBLISH_MAX_RETRY_DELAY;
            const delay = Math.min(currentDelay * iteration, maxDelay);
