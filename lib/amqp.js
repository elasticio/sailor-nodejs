...
class Amqp {
    ...
    async decryptMessage(message) {
        const maesterId = message.properties.headers.maesterId;
        let content = message.content;

        if (maesterId) {
            const options = {
                auth: { bearer: this.settings.MAESTER_JWT },
                forever: true,
                maxAttempts: 3,
                retryDelay: 100
            };
            const res = (await request.get(`${this.settings.MAESTER_BASEPATH}/objects/${maesterId}`, options));
            if (res.statusCode >= 400) {
                throw new Error(`HTTP error during maester get: ${res.statusCode} (${res.statusMessage})`);
            }
            content = res.body;
        }

        return encryptor.decryptMessageContent(content);
    }
    ...
    async encryptMessage(data, properties) {
        ...
        const options = {
            ...
            retryDelay: 100,
            body: encryptedData,
            retryStrategy: (err, response, body, options) => {
                if (!err && response.statusCode === 409) {
                    maesterId = uuid.v4();
                    options.url = `${this.settings.MAESTER_BASEPATH}/objects/${maesterId}`;
                    return { mustRetry: true, options };
                }
                return request.RetryStrategies.HTTPOrNetworkError(err, response, body, options);
            }
        };
        const res = await request.put(`${this.settings.MAESTER_BASEPATH}/objects/${maesterId}`, options);
        if (res.statusCode >= 400) {
            throw new Error(`HTTP error during maester put: ${res.statusCode} (${res.statusMessage})`);
        }
