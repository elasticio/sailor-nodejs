...
function AMQPConnection() {
    ...
    this.PREFETCH_COUNT = settings.get('RABBITMQ_PREFETCH_SAILOR');
    this.PUBLISH_MESSAGES_TO = settings.get('PUBLISH_MESSAGES_TO');

    this.DATA_ROUTING_KEY = settings.get('DATA_ROUTING_KEY');
    this.ERROR_ROUTING_KEY = settings.get('ERROR_ROUTING_KEY');
    this.SNAPSHOT_ROUTING_KEY = settings.get('SNAPSHOT_ROUTING_KEY');
    this.REBOUND_ROUTING_KEY = settings.get('REBOUND_ROUTING_KEY');

    this.REBOUND_INITIAL_EXPIRATION = settings.get('REBOUND_INITIAL_EXPIRATION');
    this.REBOUND_LIMIT = settings.get('REBOUND_LIMIT');
}
...
AMQPConnection.prototype.listenQueue = function listenQueue(queueName, callback) {
    ...
    this.subscribeChannel.prefetch(this.PREFETCH_COUNT);
...
AMQPConnection.prototype.sendData = function sendData(data, headers) {
    ...
    this.sendToExchange(this.PUBLISH_MESSAGES_TO, this.DATA_ROUTING_KEY, encryptedData, options);
};
...
AMQPConnection.prototype.sendError = function sendError(err, headers, originalMessageContent) {
    ...
    this.sendToExchange(this.PUBLISH_MESSAGES_TO, this.ERROR_ROUTING_KEY, errorPayload, options);
};
...
AMQPConnection.prototype.sendRebound = function sendRebound(reboundError, originalMessage, headers) {
    ...
    if (reboundIteration > that.REBOUND_LIMIT) {
        ...
        this.sendToExchange(
            this.PUBLISH_MESSAGES_TO,
            this.REBOUND_ROUTING_KEY,
            originalMessage.content,
            options
        );
    }
    ...
    function getExpiration(iteration) {
        return Math.pow(2, iteration - 1) * that.REBOUND_INITIAL_EXPIRATION;
    }
};

AMQPConnection.prototype.sendSnapshot = function sendSnapshot(data, headers) {
    var options = getOptions(headers);
    var exchange = this.PUBLISH_MESSAGES_TO;
    var routingKey = this.SNAPSHOT_ROUTING_KEY;
    try {
