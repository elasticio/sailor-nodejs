...
AMQPConnection.prototype.listenQueue = function listenQueue(queueName, callback) {
    ...
    this.subscribeChannel.prefetch(env.get('RABBITMQ_PREFETCH_SAILOR'));
...
AMQPConnection.prototype.sendData = function sendData(data, headers) {
    ...
    console.log('sendToExchange');

    this.sendToExchange(env.get('PUBLISH_MESSAGES_TO'), env.get('DATA_ROUTING_KEY'), encryptedData, options);

    console.log('sentToExchange');
};
...
AMQPConnection.prototype.sendError = function sendError(err, headers, originalMessageContent) {
    ...
    this.sendToExchange(env.get('PUBLISH_MESSAGES_TO'), env.get('ERROR_ROUTING_KEY'), errorPayload, options);
};
...
AMQPConnection.prototype.sendRebound = function sendRebound(reboundError, originalMessage, headers) {
    ...
    if (reboundIteration > env.get('REBOUND_LIMIT')) {
        ...
        this.sendToExchange(
            env.get('PUBLISH_MESSAGES_TO'),
            env.get('REBOUND_ROUTING_KEY'),
            originalMessage.content,
            options
        );
    }
    ...
    function getExpiration(iteration) {
        return Math.pow(2, iteration - 1) * env.get('REBOUND_INITIAL_EXPIRATION');
    }
};

AMQPConnection.prototype.sendSnapshot = function sendSnapshot(data, headers) {
    var options = getOptions(headers);
    var exchange = env.get('PUBLISH_MESSAGES_TO');
    var routingKey = env.get('SNAPSHOT_ROUTING_KEY');
    try {
