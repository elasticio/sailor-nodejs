...
class Amqp {
    ...
    async prepareMessageAndSendToExchange(data, properties, routingKey, throttle) {
        ...
        const protocolVersion = Number(this.settings.PROTOCOL_VERSION || 1);
    ...
    async sendError(err, properties, originalMessageContent, throttle) {
        ...
        if (originalMessageContent && originalMessageContent.length) {
            payload.errorInput = originalMessageContent.toString('base64');
        }
        const errorPayload = JSON.stringify(payload);

        let errProps = _.cloneDeep(properties);
        errProps.headers.protocolVersion = 1;
        ...
        if (headers.reply_to) {
            ...
            replyToOptions.headers.protocolVersion = 1;
