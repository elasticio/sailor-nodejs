...
class Amqp {
    ...
    async disconnect() {
        log.debug('Close AMQP connections');
    ...
    async listenQueue(queueName, callback) {
        ...
        const result = await this.subscribeChannel.consume(queueName, this._onMessage.bind(this, callback));
        this.consumerTag = result.consumerTag;
        return result;
    }
    _onMessage(callback, amqpMessage) {
        if (amqpMessage === null) {
            // this might happen when connection is closed.
        ...
        try {
            ...
            return this.reject(amqpMessage);
        }
        try {
            callback(message, amqpMessage);
        } catch (e) {
    ...
    async listenQueueCancel() {
        log.debug('listenQueueCancel starting');
        if (!this.consumerTag) {
    ...
    decryptMessage(callback, message) {
        ...
        try {
            ...
            log.trace(err, 'Failed to process message #%j, reject', message.fields.deliveryTag);
