...
class Amqp {
    ...
    listenQueue(queueName, callback) {
        ...
        return this.subscribeChannel.consume(queueName, async function decryptMessage(message) {
            ...
            // TODO: handle errors
            // TODO: check is_maester flag is not copied to outgoing message
            const content = message.properties.headers.is_maester ? await maesterGet(message.properties.headers.messageId) : message.content;
    ...
    async prepareMessageAndSendToExchange(data, properties, routingKey, throttle) {
        ...
        if (this.settings.MAESTER_IS_STORE) {
            // TODO: handle errors
            await maesterPut(data.headers.messageId, encryptedData);
            properties.headers.is_maester = true;
        }
