...
var TIMEOUT = env.get('TIMEOUT') || 20 * 60 * 1000; // 20 minutes
...
function Sailor() {
    ...
    this.apiClient = new ApiClient(env.get('API_URI')).auth(env.get('API_USERNAME'), env.get('API_KEY'));
}

Sailor.prototype.connect = function connect() {
    return Q.all([
        this.componentReader.init(env.get('COMPONENT_PATH')),
        this.amqpConnection.connect(env.get('AMQP_URI'))
    ]);
};
...
Sailor.prototype.readIncomingMessageHeaders = function readIncomingMessageHeaders(message) {
    ...
    if (env.get('TASK')._id && headers.taskId !== env.get('TASK')._id) {
