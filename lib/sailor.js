...
const EIO_HEADER_FLOW_ID = 'x-eio-flow-id';
        ...
        function onData(data) {
            ...
            if (data.headers && Object.keys(data.headers).length > 0) {
                for(const key in data.headers) {
                    if (key.toLowerCase() === EIO_HEADER_FLOW_ID) {
                        // Sometimes we need to override the task ID
                        // in RabbitMQ header
                        debug('Found alternative flow-id in message header flowId=%s', data.headers[key]);
                        headers.taskId = data.headers[key];
            ...
            debug('RabbitMQ message headers will be headers=%j', headers);
