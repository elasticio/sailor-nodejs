...
class Sailor {
    ...
    reportError(err) {
        const headers = Object.assign({}, getAdditionalHeadersFromSettings(this.settings), {
            exec_id: this.settings.EXEC_ID,
            task_id: this.settings.FLOW_ID,
            workspace_id: this.settings.WORKSPACE_ID,
            container_id: this.settings.CONTAINER_ID,
            user_id: this.settings.USER_ID,
            step_id: this.settings.STEP_ID,
            comp_id: this.settings.COMP_ID,
    ...
    async runExec(module, payload, incomingMessageHeaders, outgoingMessageHeaders, stepData, timeStart, logger) {
        ...
        await new Promise(resolve => {
            ...
            const taskExec = new TaskExec({
                loggerOptions: _.pick(incomingMessageHeaders, ['thread_id', 'message_id', 'parent_message_id']),
            ...
            async function onData(data) {
                ...
                headers.message_id = data.id || headers.message_id;
                logger.trace({
                ...
                if (stepData.is_passthrough === true) {
                    ...
                    if (settings.NO_SELF_PASSTRHOUGH) {
                        const { step_id } = incomingMessageHeaders;
                        if (step_id) {
                            data.passthrough = Object.assign({}, origPassthrough, {
                                [step_id]: Object.assign({}, _.omit(payload, 'passthrough'))
                            });
                        }
                    }
                }
                ...
                try {
                    await that.proxyClient.sendMessage({
                        incomingMessageId: incomingMessageHeaders.message_id,
                        data,
                        headers,
                        type: 'data'
                    });
            ...
            async function onRebound(err) {
                ...
                return that.proxyClient.sendRebound(err, payload, outgoingHeaders);
            }
            ...
            async function onUpdateKeys(keys) {
                ...
                try {
                    ...
                    logger.debug({ message_id: incomingMessageHeaders.message_id }, 'Successfully updated keys');
                } catch (error) {
                    logger.debug({ message_id: incomingMessageHeaders.message_id }, 'Failed to update keys');
                    await onError(error);
                }
            }
    ...
    async processMessage(headers, payload) {
        ...
        const logger = log.child({
            thread_id: headers.thread_id || 'unknown',
            message_id: headers.message_id || 'unknown',
            parent_message_id: headers.parent_message_id || 'unknown'
        });
        ...
        const outgoingMessageHeaders = {
            ...
            exec_id: settings.EXEC_ID,
            task_id: settings.FLOW_ID,
