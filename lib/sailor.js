...
class Sailor {
    ...
    async uploadMessageBody(body) {
        ...
        if (buf.length <= OBJECT_STORAGE_SIZE_THRESHOLD) {
            return;
        }
        ...
        await new Promise(resolve => {
            ...
            async function onData(data) {
                ...
                if (settings.EMIT_LIGHTWEIGHT_MESSAGE) {
                    await Promise.all([
                        (async () => {
                            const id = await that.uploadMessageBody(body);
                            if (id) {
                                const { headers } = data;
                                data.body = {};
                                data.headers = {
                        ...
                        ...(
                            passthrough
                                ? Object.keys(passthrough).map(async stepId => {
                                    ...
                                    const id = await that.uploadMessageBody(data.passthrough[stepId].body);
                                    if (id) {
