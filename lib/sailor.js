...
class Sailor {
    ...
    async fetchMessageBody(message, logger) {
        ...
        logger.trace('Checking if incoming messages is lightweight...', { headers });

        if (!headers) {
            logger.debug('Empty headers so not lightweight.');
            logger.trace('Empty headers so not lightweight.', { headers });
            return body;
        }
        ...
        if (!objectId) {
            ...
            logger.trace('No object id header so not lightweight.', { headers });
            return body;
        }

        logger.debug('Object id header found, message is lightweight.');
        logger.trace('Object id header found, message is lightweight.', { objectId });

        let object;

        logger.info('Going to fetch message body.');
        logger.debug('Going to fetch message body.', { objectId });
            ...
            async function onData(data) {
                ...
                if (settings.EMIT_LIGHTWEIGHT_MESSAGE) {
                    ...
                    if (totalLength > settings.OBJECT_STORAGE_SIZE_THRESHOLD) {
                        ...
                        if (bodyId) {
                            logger.debug('Message body uploaded', { id: bodyId });
                            const { headers } = data;
                            data.body = {};
                            data.headers = {
                                ...(headers || {}),
                        ...
                        for (const { stepId, bodyId } of passthroughIds) {
                            if (bodyId) {
                                logger.debug('Passthrough Message body uploaded', { stepId, id: bodyId });
                                ...
                                logger.debug('Passtrough Message body not uploaded', { stepId, id: bodyId });
                            }
                        }
                    ...
                    try {
                        await Promise.all(Object.keys(passthrough).map(async stepId => {
                            logger.debug('Going to check if passthrough for step is lightweight.', { stepId });
                            ...
                            if (!_.isEmpty(data.passthrough[stepId].body)) {
                                logger.debug('Body is not empty.', { stepId });
                                return;
                            }
