...
class Sailor {
    ...
    async fetchMessageBody(message) {
    ...
    runExec(module, payload, message, outgoingMessageHeaders, stepData, timeStart, logger) {
            ...
            async function onData(data) {
                ...
                if (stepData.is_passthrough === true) {
                    if (settings.NO_SELF_PASSTRHOUGH) {
                        ...
                        data.passthrough = Object.assign({}, origPassthrough, {
                            [that.settings.STEP_ID]: Object.assign({}, _.omit(data, 'passthrough'))
                        });
                    }
                }

                try {
                    return await that.amqpConnection.sendData(data, headers, that.throttles.data);
                } catch (err) {
                    return onError(err);
                }
            }
    ...
    async processMessage(payload, message) {
        ...
        try {
            await Promise.all([
                (async () => {
                    payload.body = await this.fetchMessageBody(payload);
                })(),
                ...(passthrough
                    ? Object.keys(passthrough).map(async stepId => {
                        payload.passthrough[stepId].body = await this.fetchMessageBody(payload.passthrough[stepId]);
                    })
                    : [])
            ]);
        } catch (e) {
        ...
        return this.runExec(module, payload, message, outgoingMessageHeaders, stepData, timeStart, logger);
    }
}
