...
class Sailor {
    ...
    processMessage(payload, message) {
        ...
        function processMessageWithModule(module) {
            ...
            taskExec
                .on('data', pThrottle(onData, self.settings.DATA_RATE_LIMIT, 1000))
                .on('error', pThrottle(onError, self.settings.ERROR_RATE_LIMIT, 1000))
                .on('rebound', onRebound)
                .on('snapshot', pThrottle(onSnapshot, self.settings.SNAPSHOT_RATE_LIMIT, 1000))
            ...
            async function onData(data) {
                ...
                await self.amqpConnection.sendData(data, props, self.throttles.data);
            }
            ...
            async function onHttpReply(reply) {
                ...
                await self.amqpConnection.sendHttpReply(reply, props);
            }
            ...
            async function onError(err) {
                ...
                headers.end = new Date().getTime();
                const props = createAmqpProperties(headers);
                await self.amqpConnection.sendError(err, props, message.content);
            }
            ...
            async function onRebound(err) {
                ...
                const props = createAmqpProperties(headers);
                await self.amqpConnection.sendRebound(err, message, props);
            }
            ...
            async function onSnapshot(data) {
                ...
                const props = createAmqpProperties(headers);
                await self.amqpConnection.sendSnapshot(data, props);
            }
