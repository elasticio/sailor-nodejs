...
var TIMEOUT = settings.get('TIMEOUT') || 20 * 60 * 1000; // 20 minutes
...
function Sailor() {
    ...
    this.apiClient = new ApiClient(settings.get('API_URI')).auth(settings.get('API_USERNAME'), settings.get('API_KEY'));
}

Sailor.prototype.connect = function connect() {
    return Q.all([
        this.componentReader.init(settings.get('COMPONENT_PATH')),
        this.amqpConnection.connect(settings.get('AMQP_URI'))
    ]);
};
...
Sailor.prototype.getStepCfg = function getStepCfg(stepId) {
    return _.cloneDeep(settings.get('TASK').data[stepId]) || {};
};

Sailor.prototype.getStepSnapshot = function getStepSnapshot() {
    var task = settings.get('TASK');
    var stepId = settings.get('STEP_ID');
    return _.get(task, ['snapshot', stepId]) ? task.snapshot[stepId] : {};
};

Sailor.prototype.getStepInfo = function getStepInfo(stepId) {
    return _.find(settings.get('TASK').recipe.nodes, {'id': stepId});
};

Sailor.prototype.run = function run() {
    var incomingQueue = settings.get('LISTEN_MESSAGES_ON');
...
Sailor.prototype.readIncomingMessageHeaders = function readIncomingMessageHeaders(message) {
    ...
    if (settings.get('TASK')._id && headers.taskId !== settings.get('TASK')._id) {
