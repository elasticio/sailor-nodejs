...
class Sailor {
    ...
    isConnected() {
        return this.proxyClient.isHealthy();
    }
    ...
    async prepare() {
        ...
        const {
            settings: {
                ...
                // TODO: check if retry flow works with actual retry flow id, not the original flow id
                FLOW_ID: flowId,
                STEP_ID: stepId
            },
            apiClient,
        ...
        await new Promise(resolve => {
            ...
            async function onData(data) {
                ...
                if (settings.EMIT_LIGHTWEIGHT_MESSAGE) {
                    ...
                    const bodyLength = Buffer.from(JSON.stringify(body), 'utf-8').length;
                    const passthroughData = Object.keys(passthrough).map(stepId => ({
                        stepId,
                        body: passthrough[stepId].body,
                        bodyLength: Buffer.from(JSON.stringify(passthrough[stepId].body), 'utf-8').length,
                        id: passthrough[stepId].headers && passthrough[stepId].headers[OBJECT_ID_HEADER]
                    }));

                    const totalLength = passthroughData.reduce((len, { bodyLength }) =>
                        len + bodyLength, bodyLength);
                    ...
                    if (totalLength > settings.OBJECT_STORAGE_SIZE_THRESHOLD) {
                        ...
                        try {
                            [bodyId, ...passthroughIds] = await Promise.all([
                                that.uploadMessageBody(body),
                                ...passthroughData.map(async ({ stepId, body, id }) => {
    ...
    async processMessage(metadata, payload) {
        ...
        try {
            ...
            await self.proxyClient.sendError(e, outgoingMetadata, payload, metadata);
            await self.proxyClient.finishProcessing(metadata, MESSAGE_PROCESSING_STATUS.ERROR);
            return;
        }
