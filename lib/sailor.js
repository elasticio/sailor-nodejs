...
class Sailor {
    ...
    async prepare() {
        ...
        log.trace('Received step data: %j', stepData);
        assert(stepData);
    ...
    async processMessageAndMaybeShutdownCallback(payload, message) {
        try {
            ...
            log.trace(e, 'Something very bad happened during message processing');
    ...
    async fetchMessageBody(message, logger) {
        ...
        logger.debug('Checking if incoming messages is lightweight...');
        logger.trace({ headers }, 'Checking if incoming messages is lightweight...');

        if (!headers) {
            logger.debug('Empty headers so not lightweight.');
            logger.trace({ headers }, 'Empty headers so not lightweight.');
            return body;
        }
        ...
        if (!objectId) {
            logger.debug('No object id header so not lightweight.');
            logger.trace({ headers }, 'No object id header so not lightweight.');
            return body;
        }

        logger.debug('Object id header found, message is lightweight.');
        logger.trace({ objectId }, 'Object id header found, message is lightweight.');

        let object;

        logger.info('Going to fetch message body.');
        logger.debug({ objectId }, 'Going to fetch message body.');
        ...
        logger.debug({ objectId }, 'Successfully obtained message body.');
    ...
    async uploadMessageBody(bodyBuf) {
        ...
        try {
            id = await this.objectStorage.addAsStream(Readable.from(bodyBuf) ,this.settings.OBJECT_STORAGE_TOKEN);
        } catch (e) {
            log.trace(e);
            log.error('Error occurred during uploading message');
        }
        ...
        await new Promise(resolve => {
            ...
            async function onData(data) {
                ...
                if (settings.EMIT_LIGHTWEIGHT_MESSAGE) {
                    logger.info('Outgoing lightweight is enabled, going to check size.');
                    ...
                    if (totalLength > settings.OBJECT_STORAGE_SIZE_THRESHOLD) {
                        ...
                        if (bodyId) {
                            logger.debug({ id: bodyId }, 'Message body uploaded');
                            const { headers } = data;
                            data.body = {};
                            data.headers = {
                                ...(headers || {}),
                            ...
                            logger.debug('Message body not uploaded');
                        }

                        for (const { stepId, bodyId } of passthroughIds) {
                            if (bodyId) {
                                logger.debug({ stepId, id: bodyId }, 'Passthrough Message body uploaded');
                                ...
                                logger.debug({ stepId, id: bodyId }, 'Passtrough Message body not uploaded');
                            }
                        }
                    ...
                    logger.info('Outgoing lightweight is disabled, going to download all bodies.');
                    try {
                        await Promise.all(Object.keys(passthrough).map(async stepId => {
                            logger.debug({ stepId }, 'Going to check if passthrough for step is lightweight.');
                            ...
                            if (!_.isEmpty(data.passthrough[stepId].body)) {
                                logger.debug({ stepId }, 'Body is not empty.');
                                return;
                            }
    ...
    async processMessage(payload, message) {
        ...
        if (method.autoResolveObjectReferences) {
            ...
            try {
                await Promise.all([
                    (async () => {
                        logger.info('Going to check if incoming message body is lightweight.');
