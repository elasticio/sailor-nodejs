...
class Sailor {
    ...
    constructor(settings) {
        ...
        this.objectStorage = objectStorage.use(encryptor.createCipher(), encryptor.createDecipher());
    ...
    async uploadMessageBody(bodyBuf) {
        const readable = Readable.from(bodyBuf.values());
            ...
            async function onData(data) {
                ...
                if (settings.EMIT_LIGHTWEIGHT_MESSAGE) {
                    ...
                    const passthroughBufs = Object.keys(passthrough).map(stepId => ({
                        ...
                        id: passthrough[stepId].headers[OBJECT_ID_HEADER]
                    }));

                    const totalLength = bodyBuf.length
                        + passthroughBufs.reduce((len, { body }) => len + body.length, 0);
