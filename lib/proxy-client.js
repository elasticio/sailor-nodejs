    ...
    async uploadMessageBody(body) {
        return this._proxyRequestWithRetries('Upload message body', () => new Promise((resolve, reject) => {
            ...
            const postMessageStream = this.clientSession.request({
                ...
                [HTTP2_HEADER_METHOD]: 'POST',
                [HTTP2_HEADER_CONTENT_LENGTH]: encryptedBody.length,
                [HTTP2_HEADER_AUTHORIZATION]: this.authHeader
            });
            ...
            postMessageStream.on('error', (err) => {
                ...
                if (postMessageStream.rstCode === NGHTTP2_ENHANCE_YOUR_CALM) {
                    reject(new Error('Rate limit exceeded or message size is too big while uploading message body'));
                    return;
                }
                err.isNetworkError = true;
                reject(err);
