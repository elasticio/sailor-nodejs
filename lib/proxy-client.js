    ...
    async listenForMessages(messageHandler) {
        while (this.listeningForMessages) {
            try {
                        ...
                        const { headers, body } = await new Promise((resolve, reject) => {
                            getMessageStream.on('response', (headers, flags) => {
                                ...
                                if (headers[HTTP2_HEADER_STATUS] === 204) {
                                    ...
                                    // TODO: test if this fires the 'close' event
                            ...
                            getMessageStream.on('close', (...args) => {
                                console.log('Message stream close event', ...args);
                ...
                if (this.reconnecting) {
                    log.debug('Currently reconnecting, waiting before retrying message listening');
                    // Wait longer during reconnection to avoid busy looping
                    await new Promise(resolve => setTimeout(resolve, 5000));
                } else {
                    log.error(err, 'Error in listenForMessages, waiting before retry');
