...
const PROXY_HEADERS_QUERY_PARAM = 'proxyHeaders';
...
class ProxyClient {
    ...
    _extractProxyHeaders(proxyHeaders) {
        ...
        Object.entries(proxyHeaders || {}).forEach(([key, value]) => {
            ...
            if (originalKey.toLowerCase().startsWith(AMQP_HEADER_META_PREFIX)) {
                metaHeaders[originalKey.toLowerCase()] = value;
            } else {
                headers[_.camelCase(originalKey)] = value;
            }
        });
        ...
        const result = {
            ...
            ...metaHeaders,
            threadId: headers.threadId || metaHeaders['x-eio-meta-trace-id'],
