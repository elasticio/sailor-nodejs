...
class ProxyClient {
    constructor(settings) {
        ...
        if (!this.settings.PROXY_CLIENT_ID) {
            throw new Error('PROXY_CLIENT_ID must be set to connect to Sailor Proxy');
        }
        ...
        this.proxyJWT = sign({
            ...
            clientId: settings.PROXY_CLIENT_ID,
    ...
    _getSessionOptions() {
        const messageLimitMB = this.settings.OUTGOING_MESSAGE_SIZE_LIMIT / (1024 * 1024); // Convert bytes to MB
        // Set max session memory to 1.2x the message limit, but at least 10MB (default http2 value)
        const maxSessionMemory = Math.max(10, Math.ceil(messageLimitMB * 1.2));
        log.debug({ maxSessionMemory }, 'Setting max session memory for http/2 server');
        return { maxSessionMemory };
    }

    async connect() {
        try {
            log.trace('connecting to http2 server');
            this.clientSession = http2.connect(this.settings.SAILOR_PROXY_URI, this._getSessionOptions());
            this.closed = false;
            this.reconnectAttempts = 0;
            ...
            log.debug('Successfully connected to Sailor Proxy');
    ...
    _scheduleReconnect() {
        if (this.closed) {
            log.debug('Connection closed intentionally, skipping reconnection');
            this.reconnecting = false;
            return;
        }
        ...
        log.debug({ attempt: this.reconnectAttempts, baseDelayMs: baseDelay, jitteredDelayMs: delay }, 'Scheduling reconnection attempt');

        this.reconnectTimer = setTimeout(async () => {
            try {
                log.debug({ attempt: this.reconnectAttempts }, 'Attempting to reconnect');
                await this._reconnect();
                log.debug('Successfully reconnected to Sailor Proxy, going to resume processing in-flight messages');
    ...
    _startPingInterval() {
        ...
        const intervalMs = this.settings.PROXY_PING_INTERVAL_MS;
        this._pingInterval = setInterval(() => {
            if (!this.clientSession || this.clientSession.destroyed) {
                return;
            }
            this.clientSession.ping((err, duration) => {
                if (err) {
                    log.debug({ err }, 'HTTP/2 ping failed, connection likely dead');
                    if (!this.closed) {
                        this._handleDisconnection('ping_timeout', err);
                    }
                } else {
                    log.trace({ duration }, 'HTTP/2 ping successful');
                }
            });
        }, intervalMs);
        log.trace({ intervalMs }, 'Started HTTP/2 ping interval');
    }
    ...
    _stopPingInterval() {
        if (this._pingInterval) {
            ...
            log.debug('Stopped HTTP/2 ping interval');
        }
    }
    ...
    async disconnect() {
        ...
        // Abort any pending throttled sends so their setTimeout callbacks don't
        // fire after the session is gone and pollute subsequent test stubs.
        for (const throttle of Object.values(this.throttles)) {
            try {
                throttle.abort();
    ...
    async _ensureConnection() {
        ...
        if (this.reconnecting) {
            ...
            log.debug('Waiting for reconnection to complete');
            const maxWait = 30000; // 30 seconds (TODO: make this configurable)
    ...
    async _proxyRequestWithRetries(operationName, requestFn) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt > 0) {
                    log.debug({ attempt, maxRetries }, `${operationName} succeeded after retry`);
                }
                return result;
            } catch (error) {
                ...
                if (!this.isConnected()) {
                    log.debug('Reconnecting before retry...');
                    await this.connect();
                }
            }
        }
    ...
    async resumeProcessing() {
        ...
        log.debug({ numberOfMessages: messagesToResume.length }, 'Resuming processing of in-flight messages after reconnection');
        if (messagesToResume.length === 0) {
            log.debug('No in-flight messages to resume');
            return;
        }
        ...
        await this._proxyRequestWithRetries('Resume processing', () => new Promise((resolve, reject) => {
            ...
            postMessageStream.on('response', (headers) => {
                ...
                const isSuccess = headers[HTTP2_HEADER_STATUS] === 200;
                const chunks = [];
                if (!isSuccess) {
                    log.error({ headers }, 'Failed to resume processing');
                    postMessageStream.on('data', (chunk) => chunks.push(chunk));
                }
                postMessageStream.on('end', () => {
                    ...
                    log.error({ body, statusCode: headers[HTTP2_HEADER_STATUS] }, 'Error response body from resume processing request');
                    process.exit(1); // Exit immediately as we cannot resume processing, and continuing to run would result in duplicate processing
                });
            });
            ...
            postMessageStream.on('close', () => {
                const isConnected = this.isConnected();
                log.debug({ rstCode: postMessageStream.rstCode, isConnected }, 'Resume processing stream closed');
                if (!isConnected) {
                    log.warn('Resume processing stream closed due to lost connection, will retry on next reconnection');
                    const error = new Error('Resume processing stream closed due to lost connection');
                    error.isNetworkError = true;
                    reject(error);
                } else if (postMessageStream.rstCode !== NGHTTP2_NO_ERROR) {
                    log.error({ rstCode: postMessageStream.rstCode }, 'Resume processing stream closed with error');
                    const error = new Error(`Resume processing stream closed with error, rstCode: ${postMessageStream.rstCode}`);
                    reject(error);
                } else {
                    resolve();
                }
            });
            postMessageStream.on('aborted', () => {
                log.debug('Resume processing stream aborted');
            });
            postMessageStream.on('timeout', () => {
                log.debug('Resume processing stream timeout');
            });
        }));
        log.debug('Successfully resumed processing of in-flight messages');
    }

    async listenForMessages(messageHandler) {
        while (this.listeningForMessages) {
            try { log.debug('Starting to listen for messages from proxy');
                await this._ensureConnection();
                ...
                await Promise.all(new Array(prefetch).fill().map(async () => {
                    ...
                    log.debug({ prefetch }, 'Requesting message from proxy');
                    ...
                    try {
                        const { headers, body } = await new Promise((resolve, reject) => {
                            getMessageStream.on('response', (headers, flags) => {
                                log.debug({ headers, flags }, 'Connected to message stream');
                                const chunks = [];
                                getMessageStream.on('data', chunk => {
                                    chunks.push(chunk);
                                });
                                if (headers[HTTP2_HEADER_STATUS] === 204) {
                                    log.debug('Received empty message stream, closing stream and going to request again');
                                    getMessageStream.close(NGHTTP2_NO_ERROR);
                                    return;
                                } else if (headers[HTTP2_HEADER_STATUS] !== 200) {
                                    log.error({ headers }, 'Failed to get message');
                                }
                                getMessageStream.on('end', () => {
                                    log.debug('Message stream ended by server');
                                    ...
                                    if (headers[HTTP2_HEADER_STATUS] === 204) {
                                        ...
                                        const defaultErrorMessage = `Failed to send message, status code: ${headers[HTTP2_HEADER_STATUS]}`;
                                        const error = new Error(body || defaultErrorMessage);
                            ...
                            getMessageStream.on('close', () => {
                                const isConnected = this.isConnected();
                                log.warn({ rstCode: getMessageStream.rstCode, isConnected }, 'Message stream closed by server');
                                if (!isConnected) {
                                    log.warn('Message stream closed due to lost connection, will retry on next reconnection');
                                    const error = new Error('Message stream closed due to lost connection');
                                    error.isNetworkError = true;
                                    reject(error);
                                } else if (getMessageStream.rstCode !== NGHTTP2_NO_ERROR) {
                                    const error = new Error(`Message stream closed with error, rstCode: ${getMessageStream.rstCode}`);
                                    reject(error);
                                } else {
                                    resolve({ headers: null, body: null });
                                }
                            ...
                            getMessageStream.on('aborted', () => {
                                log.warn('Message stream aborted by server');
                            });
                            getMessageStream.on('timeout', () => {
                                log.warn('Message stream timeout');
                            });
                        });
                ...
                if (this.closed) {
                    log.debug('Connection closed, stopping message listener');
                    break;
                }
    ...
    async stopListeningForMessages() {
        log.debug('Stopping listening for messages');
    ...
    _prepareData(data, metadata, type, forceProtocolVersion) {
        ...
        if (protocolVersion && data) {
            ...
            delete preparedMetadata.protocolVersion;
        }
        return {
            preparedMetadata,
            preparedData
    ...
    async sendMessage({
        ...
        const preparedDataSize = preparedData ? preparedData.length : null;
        log.debug({ preparedMetadata, preparedDataSize: preparedData ? preparedData.length : null }, 'Prepared message for sending to proxy');

        const messageSizeLimit = this.settings.OUTGOING_MESSAGE_SIZE_LIMIT;
        // Check the size of the message before sending it to proxy to avoid NGHTTP2_ENHANCE_YOUR_CALM errors from client's http2 session
        // (if we do not do this here and rely on proxy to reject request with 413, the client session will hit the maxSessionMemory
        // limit and return NGHTTP2_ENHANCE_YOUR_CALM error, which will block all subsequent requests until the session is closed and re-established)
        if (preparedDataSize && preparedDataSize > messageSizeLimit) {
            log.error({ preparedDataSize, messageSizeLimit }, 'Outgoing message size exceeds limit');
            throw new Error(`Outgoing message size ${preparedDataSize} exceeds limit of ${messageSizeLimit}`);
        }
        ...
        await this._proxyRequestWithRetries('Send message', () => new Promise((resolve, reject) => {
            const postMessageStream = this.clientSession.request({
                ...
                [HTTP2_HEADER_METHOD]: 'POST',
                [HTTP2_HEADER_AUTHORIZATION]: this.authHeader,
                [HTTP2_HEADER_CONTENT_LENGTH]: preparedData ? preparedData.length : 0,
            ...
            postMessageStream.on('close', () => {
                const isConnected = this.isConnected();
                log.warn({ rstCode: postMessageStream.rstCode, isConnected }, 'Send message stream closed by server');
                if (!isConnected) {
                    log.warn('Send message stream closed due to lost connection, will retry on next reconnection');
                    const error = new Error('Send message stream closed due to lost connection');
                    error.isNetworkError = true;
                    reject(error);
                } else if (postMessageStream.rstCode !== NGHTTP2_NO_ERROR) {
                    log.error({ rstCode: postMessageStream.rstCode }, 'Send message stream closed with error');
                    const error = new Error(`Send message stream closed with error, rstCode: ${postMessageStream.rstCode}`);
                    error.isNetworkError = !this.isConnected(); // Mark as network error if the connection is not active to retry the request
                    reject(error);
                } else {
                    resolve();
                }
            ...
            postMessageStream.on('aborted', () => {
                log.debug('Send message stream aborted');
            });
            postMessageStream.on('timeout', () => {
                log.debug('Send message stream timeout');
            });
        }));
    }
        ...
        await this._proxyRequestWithRetries('Finish processing', () => new Promise((resolve, reject) => {
            ...
            postMessageStream.on('response', (headers) => {
                ...
                postMessageStream.on('end', () => {
                    ...
                    const body = Buffer.concat(chunks).toString('utf8');
                    const defaultErrorMessage = `Failed to finish processing message, status code: ${headers[HTTP2_HEADER_STATUS]}`;
                    const error = new Error(body || defaultErrorMessage);
                    error.statusCode = headers[HTTP2_HEADER_STATUS];
                    return reject(error);
                });
            ...
            postMessageStream.on('close', () => {
                const isConnected = this.isConnected();
                log.warn({ rstCode: postMessageStream.rstCode, isConnected }, 'Finish processing stream closed by server');
                if (!isConnected) {
                    log.warn('Finish processing stream closed due to lost connection, will retry on next reconnection');
                    const error = new Error('Finish processing stream closed due to lost connection');
                    error.isNetworkError = true;
                    reject(error);
                } else if (postMessageStream.rstCode !== NGHTTP2_NO_ERROR) {
                    log.error({ rstCode: postMessageStream.rstCode }, 'Finish processing stream closed with error');
                    const error = new Error(`Finish processing stream closed with error, rstCode: ${postMessageStream.rstCode}`);
                    error.isNetworkError = !this.isConnected(); // Mark as network error if the connection is not active to retry the request
                    reject(error);
                } else {
                    resolve();
            ...
            postMessageStream.on('aborted', () => {
                log.debug('Finish processing stream aborted');
            });
            postMessageStream.on('timeout', () => {
                log.debug('Finish processing stream timeout');
            });
        }));
    ...
    // TODO: test from old sailor.spec.js
    // https://github.com/elasticio/sailor-nodejs/pull/222/changes/0299d467642588fa55302a892b360fe7f46cdbcb#diff-de84cbf627dbbc2b42e063380c1550d349e656f01292a936f478fe712f103e15L63
