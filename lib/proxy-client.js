...
class ProxyClient {
                ...
                const { headers, body } = await new Promise((resolve, reject) => {
                    getMessageStream.on('response', (headers, flags) => {
                        ...
                        getMessageStream.on('end', () => {
                            ...
                            this.logger.info({ messageId, messageSize: body.length }, 'Received complete message from server');
                            resolve({ headers, body });
                        });
                    });
                });

                const msgHeaders = this._extractProxyHeaders(headers);
    ...
    async sendMessage({
        ...
        return new Promise((resolve, reject) => {
            req.on('response', (headers) => {
                this.logger.debug({ status: headers[HTTP2_HEADER_STATUS] }, 'Send message response');
            });
    ...
    async sendError(err, headers, originalMessage, incomingHeaders) {
        ...
        let result = await this.sendMessage({
            incomingMessageId: undefined, // TODO
            type: 'error',
            data: errorPayload,
            headers
        });
