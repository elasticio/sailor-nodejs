...
class ProxyClient {
    ...
    async resumeProcessing() {
        ...
        const queryParams = new URLSearchParams({
            clientId: this.settings.PROXY_CLIENT_ID
        }).toString();
        ...
        log.debug({ body, queryParams }, 'Request body for resuming processing');
        await this._proxyRequestWithRetries('Resume processing', () => new Promise((resolve, reject) => {
            const postMessageStream = this.clientSession.request({
                [HTTP2_HEADER_PATH]: `/resume-processing?${queryParams}`,
    ...
    async fetchMessageBody(message, logger) {
        ...
        if (!headers) {
            logger.debug('Empty headers so not lightweight.');
            return body;
        }
        ...
        logger.debug('Object id header found, message is lightweight. Going to fetch message body.', { objectId });

        const fullBody = await this._proxyRequestWithRetries('Fetch message body', () => new Promise((resolve, reject) => {
            const requestStream = this.clientSession.request({
                [HTTP2_HEADER_PATH]: `/object/${objectId}`,
                [HTTP2_HEADER_METHOD]: 'GET',
                [HTTP2_HEADER_AUTHORIZATION]: this.authHeader
            });
            // Collect raw chunks from the request stream before deciding whether to decrypt.
            // Decrypting unconditionally would fail on non-200 responses whose bodies are not encrypted.
            const chunks = [];
            requestStream.on('data', chunk => {
                chunks.push(chunk);
            });
            // 'response' is an HTTP/2-specific event emitted only on the original ClientHttp2Stream,
            // not on a piped Transform stream.
            requestStream.on('response', (headers) => {
                const isSuccess = headers[HTTP2_HEADER_STATUS] === 200;
                if (!isSuccess) {
                    log.error({ headers }, 'Failed to fetch message body');
                }
                requestStream.on('end', () => {
                    log.debug('Fetch message body end event');
                    ...
                    if (isSuccess) {
                        ...
                        try {
                            const responseJson = this._encryptor.decryptMessageContent(rawBuffer);
                            logger.debug({ messageSize: rawBuffer.length }, 'Fetched and decrypted message body successfully');
                            return resolve(responseJson);
                        } catch (e) {
                            log.error(e, 'Failed to parse fetch message body response');
                            return reject(e);
                        }
                    }
                    const body = rawBuffer.toString('utf8');
                    const defaultErrorMessage = `Failed to fetch message body, status code: ${headers[HTTP2_HEADER_STATUS]}`;
        ...
        logger.debug('Successfully obtained message body.', { objectId });
        logger.trace('Message body object received');
        return fullBody;
    }

    async uploadMessageBody(body) {
        return this._proxyRequestWithRetries('Upload message body', () => new Promise((resolve, reject) => {
            const encryptedBody = this._encryptor.encryptMessageContent(body);
            const messageSize = encryptedBody.length;
            ...
            if (messageSize > messageSizeLimit) {
                const error = new Error(`Outgoing message size ${messageSize} exceeds limit of ${messageSizeLimit}`);
                return reject(error);
            }
            ...
            const postMessageStream = this.clientSession.request({
                ...
                [HTTP2_HEADER_METHOD]: 'POST',
                [HTTP2_HEADER_CONTENT_LENGTH]: messageSize,
                [HTTP2_HEADER_AUTHORIZATION]: this.authHeader
            });
            ...
            postMessageStream.on('response', (headers, flags) => {
                const isSuccess = headers[HTTP2_HEADER_STATUS] === 200;
                if (!isSuccess) {
                    log.error({ headers }, 'Failed to upload message body');
                }
                const chunks = [];
                postMessageStream.on('data', (chunk) => chunks.push(chunk));
                postMessageStream.on('end', () => {
                    log.debug('Upload message body end event');
                    const body = Buffer.concat(chunks).toString('utf8');
                    if (isSuccess) {
                        if (!body) {
                            return resolve();
                        ...
                        try {
                            ...
                            return resolve(responseJson.objectId);
                        } catch (e) {
                            log.error(e, 'Failed to parse upload message body response');
                            return reject(e);
                        }
                    }
                    log.error({ body, statusCode: headers[HTTP2_HEADER_STATUS] }, 'Error response body from upload message body request');
                    const defaultErrorMessage = `Failed to upload message body, status code: ${headers[HTTP2_HEADER_STATUS]}`;
                    const error = new Error(body || defaultErrorMessage);
                    error.statusCode = headers[HTTP2_HEADER_STATUS];
                    return reject(error);
                });
            ...
            postMessageStream.write(encryptedBody);
            postMessageStream.end();
        }));
    }
