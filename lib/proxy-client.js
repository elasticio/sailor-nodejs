...
class ProxyClient {
    ...
    async listenForMessages(messageHandler) {
        ...
        while (!this.closed) {
            ...
            await Promise.all(new Array(prefetch).fill().map(async () => {
                ...
                log.info({ step_id: this.settings.STEP_ID, prefetch }, 'Requesting message from proxy');
                ...
                const { headers, body } = await new Promise((resolve, reject) => {
                    getMessageStream.on('response', (headers, flags) => {
                        ...
                        const message_id = headers['x-message-id'];
                        const chunks = [];
    ...
    async sendMessage({
        ...
        const encryptedData = this.encryptMessageContent(data, headers.protocol_version);
    ...
    _decodeDefaultMessage(originalMessage, headers) {
        const protocolVersion = Number(headers.protocol_version || 1);
    ...
    finishProcessing(incomingHeaders, status) {
        ...
        const incomingMessageId = incomingHeaders.message_id;
    ...
    async sendError(err, headers, originalMessage, incomingHeaders) {
        ...
        if (originalMessage) {
            const protocolVersion = Number(incomingHeaders.protocol_version || 1);
        ...
        let result = await this.sendMessage({
            incomingMessageId: incomingHeaders ? incomingHeaders.message_id : undefined,
            type: 'error',
            data: errorPayload,
            headers
        });
    ...
    _createProxyHeaders(headers) {
        headers.message_id = headers.message_id || uuid.v4();
    ...
    _extractProxyHeaders(proxyHeaders) {
        ...
        const result = {
            step_id: headers.step_id, // the only use is passthrough mechanism
            ...metaHeadersLowerCased,
            thread_id: headers.thread_id || metaHeadersLowerCased['x-eio-meta-trace-id'],
            message_id: headers.message_id,
            parent_message_id: headers.parent_message_id
        };
        if (!result.thread_id) {
            const thread_id = uuid.v4();
            log.debug({ thread_id }, 'Initiate new thread as it is not started ATM');
