...
class ProxyClient {
    ...
    async listenForMessages(queue, messageHandler) {
        ...
        while (!this.closed) {
            ...
            for (let i = 0; i < prefetch; i += 1) {
                const getMessageStream = this.clientSession.request({
                    [HTTP2_HEADER_PATH]: `/message?prefetch=${prefetch}`,
                    [HTTP2_HEADER_METHOD]: 'GET'
                });

                const { messageId, messageBuffer } = await new Promise((resolve, reject) => {
                    getMessageStream.on('response', (headers, flags) => {
                        this.logger.info({ headers, flags }, 'Connected to message stream');
                        ...
                        getMessageStream.on('end', () => {
                            this.logger.info('Message stream ended by server');
                            const buffer = Buffer.concat(chunks);
                            this.logger.info({ messageSize: buffer.length }, 'Received complete message from server');
                            resolve({ messageId, messageBuffer: buffer });
                        });
                    });
                });

                const message = this._decodeMessage(messageBuffer);
                const msgJson = JSON.parse(message);
                // Fix it
                await messageHandler(msgJson, messageId);
            }
        }
    }

    async sendMessage(messageId, stepId, type, data) {
    ...
    async sendError(err, headers, originalMessage, throttle) {
        ...
        if (!settings.NO_ERROR_REPLIES && headers.reply_to) {
            ...
            const replyToOptions = _.cloneDeep(proxyHeaders);
            replyToOptions.headers[HEADER_ERROR_RESPONSE] = true;
            result = this.sendToExchange(
                settings.PUBLISH_MESSAGES_TO,
                headers.reply_to,
                encryptedError,
                replyToOptions
