...
describe('Cipher', () => {
  ...
  beforeEach(() => {
    sinon.stub(global, 'decodeURIComponent');
    sinon.stub(global, 'encodeURIComponent');
  });

  afterEach(() => {
    global.decodeURIComponent.restore();
    global.encodeURIComponent.restore();
  });
  ...
  it('should encrypt & decrypt strings', () => {
    ...
    expect(decryptedResult.toString()).to.be.equal(content.toString());
    expect(global.decodeURIComponent).to.not.have.been.called;
    expect(global.encodeURIComponent).to.not.have.been.called;
  });
  ...
  it('should encrypt & decrypt objects', () => {
    ...
    const result = cipher.encryptMessageContent(content);
    const decryptedResult = cipher.decryptMessageContent(result);
    expect(decryptedResult).to.be.deep.equal({ property1: 'Hello world' });
  });
  ...
  it('should encrypt & decrypt buffer', () => {
    ...
    const decryptedResult = cipher.decryptMessageContent(result);
    expect(decryptedResult).to.be.equal('Hello world');
  });
  ...
  xit('should be compatible with Java-Sailor', () => {
    ...
    expect(cipher.decryptMessageContent(javaResult)).to.be.deep.equal(data);
  });
});
